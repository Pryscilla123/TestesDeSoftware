...
namespace Features.Clientes
{
    public class Cliente : Entity
    {
        ...
        public Cliente(Guid id, string nome, string sobrenome, DateTime dataNascimento,
                       DateTime dataCadastro, string email, bool ativo)
        {
            Id = id;
            Nome = nome;
            Sobrenome = sobrenome;
    ...
    public class ClienteValidacao : AbstractValidator<Cliente>
    {
        public ClienteValidacao()
        {
            RuleFor(c => c.Nome)
                ...
                .Length(2, 150).WithMessage("O nome deve ter entre {0} e {1} caracteres");

            RuleFor(c => c.Sobrenome)
                .NotEmpty().WithMessage("Por favor, certifique-se de ter inserido o sobrenome")
                .Length(2, 150).WithMessage("O sobrenome deve ter entre {0} e {1} caracteres");

            RuleFor(c => c.DataNascimento)
                .NotEmpty()
                .Must(HaveMinimumAge)
                .WithMessage("O cliente deve ter pelo menos 18 anos ou mais");
        ...
        public static bool HaveMinimumAge(DateTime dataNascimento)
        {
